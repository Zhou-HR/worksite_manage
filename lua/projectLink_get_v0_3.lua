...
local recodUnpassedStatus,recodUnpassedApps = db_query.projectLinkUnPassedRecodNum_get(proj_code)
local recodStatus,recodApps = db_query.projectLinkRecodNum_get(proj_code)

if status == true and recodStatus == true and recodUnpassedStatus == true then
  local linkStructure
  ...
  if apps ~= nil and apps[1] ~= nil then
    for k, v in pairs(apps) do
      ...
      apps[k]["proj_link_unpassed_recod_count"] = 0
      ...
      for recodAppsk, recodAppsv in pairs(recodUnpassedApps) do
        if apps[k]["proj_link_id"] == recodUnpassedApps[recodAppsk]["proj_link_id"] then
          apps[k]["proj_link_unpassed_recod_count"] = recodUnpassedApps[recodAppsk]["count"]
          break
        end
      end
    end
