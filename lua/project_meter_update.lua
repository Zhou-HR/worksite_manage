...
local user_id = tonumber(comm_func.get_http_header("user_id",ngx))
ngx.log(ngx.ERR, "appEUI user_id: ", user_id)
--649是段三峰的账号ID
if (user_id ~= 649) and (user_id ~= 1) then return end
...
status, apps = db_meter.meter_update_info(proj_code,old_meter_number,old_meter_value,new_meter_number,new_meter_value,if_receive_msg,if_use_new_box,if_state_grid,user_id)
if status == true then
	comm_func.do_dump_value(apps,0)
