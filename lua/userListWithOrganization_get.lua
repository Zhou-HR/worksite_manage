...
if paramsRight == false then
    local tab = {}
    tab["result"] = "API已停用"
    tab["error"] = error_table.get_error("ERROR_API_NOT_SUPPORT")
    ngx.say(cjson.encode(tab))
    return
end
...
if userOrganization == nil then
    status, apps, count, total = db_query.userList_get(isAdmin, user_bu_codeLike, user_id, user_name, user_mail, user_phone, user_role, user_number, user_bu_name, user_bu_code, user_job, user_code, user_entry_time, user_company, user_company_code, fuzzy_searche_key, limit, offset)
end
...
if status == true or userOrganization ~= nil then
    if userOrganization == nil then
        ...
        local user_bu_name_latest_index = 1
        ...
        for appsk, appsv in pairs(apps) do
            ...
            if userOrganization[user_company_latest_index]["user_company"] == appsv["user_company"] then
                if userOrganization[user_company_latest_index]["user_bu_list"][user_bu_name_latest_index]["user_bu_name"] == appsv["user_bu_name"] then
                    local userTab = {}
