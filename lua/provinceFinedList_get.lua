...
local status, apps, count, total, limit, offset, excelSql = db_project.provinceFinedList_get(proj_bu_code, isAdmin, proj_company_code, start_time, end_time, limit, offset)

if status == true then
    local red = redis:new()
    local nowTime = ngx.now()
    red:set(conf_sys.province_fined_list_excel_sql .. tostring(user_id), excelSql)
