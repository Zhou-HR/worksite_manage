...
--add by zhangjieqiong for gaizao 20200520
function _PROJECT.project_module_links_add_gaizao(proj_code, proj_links_before, proj_links, proj_module_name, proj_module_code, proj_link_name, proj_link_type, proj_link_name2, proj_link_type2, proj_link_name3, proj_link_type3, proj_link_name4, proj_link_type4)
    local projUpdateSql = string.format(" update tb_proj set proj_links='%s' where proj_code='%s'  returning proj_name,proj_station_type,proj_code,proj_bu_code,proj_company_code ,proj_link_pic_max_num", proj_links, proj_code)
    local pg = _PROJECT.get_new_pgmoon_connection()
    --comm_func.do_dump_value(projUpdateSql,0)
    assert(pg:connect())
    ...
    if res ~= nil and res[1] ~= nil and res[1]["proj_code"] == proj_code then
        local linkAddStatus, linkAddRes = _PROJECT.project_module_links_add_link(res[1]["proj_name"], res[1]["proj_station_type"], proj_module_name, proj_link_name, proj_link_type, proj_code, res[1]["proj_bu_code"], res[1]["proj_company_code"], res[1]["proj_link_pic_max_num"])
        local linkAddStatus, linkAddRes = _PROJECT.project_module_links_add_link(res[1]["proj_name"], res[1]["proj_station_type"], proj_module_name, proj_link_name2, proj_link_type2, proj_code, res[1]["proj_bu_code"], res[1]["proj_company_code"], res[1]["proj_link_pic_max_num"])
        local linkAddStatus, linkAddRes = _PROJECT.project_module_links_add_link(res[1]["proj_name"], res[1]["proj_station_type"], proj_module_name, proj_link_name3, proj_link_type3, proj_code, res[1]["proj_bu_code"], res[1]["proj_company_code"], res[1]["proj_link_pic_max_num"])
        local linkAddStatus, linkAddRes = _PROJECT.project_module_links_add_link(res[1]["proj_name"], res[1]["proj_station_type"], proj_module_name, proj_link_name4, proj_link_type4, proj_code, res[1]["proj_bu_code"], res[1]["proj_company_code"], res[1]["proj_link_pic_max_num"])
        if linkAddStatus == true then
            return true, res
        else
            _PROJECT.project_module_links_add_recover(proj_code, proj_links_before)
...
--add by zhangjieqiong for jiaofu 20200820
function _PROJECT.project_module_links_add_jiaofu(proj_code, proj_links_before, proj_links, proj_module_name, proj_module_code, proj_link_name, proj_link_type, proj_link_name2, proj_link_type2)
    --comm_func.do_dump_value("---------------project_module_links_add_jiaofu-----------------",0)
    local projUpdateSql = string.format(" update tb_proj set proj_links='%s' where proj_code='%s'  returning proj_name,proj_station_type,proj_code,proj_bu_code,proj_company_code ,proj_link_pic_max_num", proj_links, proj_code)
    local pg = _PROJECT.get_new_pgmoon_connection()
    --comm_func.do_dump_value(projUpdateSql,0)
    assert(pg:connect())
...
function _PROJECT.link1TuJianModuleReviewedList_get(limit, offset)
    ...
    local sql = string.format(" select proj_link_id,proj_code,proj_module_name, proj_link_name,proj_link_review_time,proj_link_type from tb_proj_link where proj_link_sync_time =0  and proj_code !='GDJZAAAAAAAAAAYD' and (proj_link_status = 3 or proj_link_status = 5 or proj_link_status = 7 or proj_link_status = 8  )  and proj_link_type=25 order by  proj_link_review_time asc limit %d offset %d ", limit, offset)
...
function _PROJECT.projectList_search(proj_code, proj_name, proj_addr, proj_establish_time, proj_station_type, proj_tower_type, proj_base_type, proj_bu_code, fuzzy_searche_key, isAdmin, proj_company_code, proj_bu_code_before, proj_module_name_ls, proj_submit_time, limit, offset)
    ...
    ngx.log(ngx.ERR, "projectList_get proj_bu_code: ", proj_bu_code)
    ngx.log(ngx.ERR, "projectList_get proj_bu_code_before: ", proj_bu_code_before)
    ...
    --正式环境注释了，测试环境也加注释，作用：按部门搜索；fixed by zhangjieqiong at 20200529
    ...
    local res = assert(pg:query(sql))
    pg:keepalive()
    ngx.log(ngx.ERR, "projectList_get projectList_search: ", sql)
...
function _PROJECT.linkModuleReviewedOkList_get(start_time, limit, offset)
    local sqlTotal = string.format("  select count(*) as total from tb_proj_link where proj_code !='GDJZAAAAAAAAAAYD' and proj_code !='GDJZAAAAAAAAABYD' and ( proj_link_status=3 or proj_link_status=5 or proj_link_status = 7 or proj_link_status = 8) and proj_link_sync_time =0 and  proj_link_review_time > %d  and (proj_link_type =25 or proj_link_type = 26 or proj_link_type = 17 or proj_link_type = 23 or proj_link_type = 24 or proj_link_type = 27)  ", start_time)
    local sql = string.format(" select proj_code,proj_module_name,proj_link_name,proj_link_type,proj_link_review_time,proj_link_id  from tb_proj_link where proj_code !='GDJZAAAAAAAAAAYD' and proj_code !='GDJZAAAAAAAAABYD' and ( proj_link_status=3 or proj_link_status=5 or proj_link_status = 7 or proj_link_status = 8) and proj_link_sync_time =0 and  proj_link_review_time > %d  and (proj_link_type =25 or proj_link_type = 26 or proj_link_type = 17 or proj_link_type = 23 or proj_link_type = 24 or proj_link_type = 27)   order by proj_link_review_time asc limit %d offset %d ", start_time, limit, offset)
...
function _PROJECT.linkModuleLinkSynced_update(proj_link_id)
    local nowTime = math.ceil(ngx.now())
    local sql = string.format(" update tb_proj_link set proj_link_sync_time=%d  where proj_link_id = %d  ", nowTime, proj_link_id)
...
function _PROJECT.linkModuleReviewedProjectList_get(start_time)
    local sql = string.format(" select proj_code   from tb_proj where proj_code !='GDJZAAAAAAAAAAYD' and proj_code !='GDJZAAAAAAAAABYD' and  proj_submit_time > %d  order by proj_submit_time asc  ", start_time)
    local pg = _PROJECT.get_new_pgmoon_connection()
...
function _PROJECT.linkModuleReviewedOkJieDianList_get(start_time, limit, offset)
    local projStatus, projApp = _PROJECT.linkModuleReviewedProjectList_get(start_time)
    local projCodeStr = nil
    if projStatus == true and projApp ~= nil and projApp[1] ~= nil then
        local projCodeTable = {}
        for appsk, appsv in pairs(projApp) do
            table.insert(projCodeTable, " '" .. appsv["proj_code"] .. "' ")
        end
        projCodeStr = table.concat(projCodeTable, ",")
    end

    if projCodeStr ~= nil then

        local sqlTotal = string.format("  select count(*) as total from tb_proj_link where proj_code in(%s) and ( proj_link_status=3 or proj_link_status=5 or proj_link_status = 7 or proj_link_status = 8) and proj_link_sync_time =0  and (proj_link_type =18 or proj_link_type = 19 ) group by proj_code,proj_module_name   ", projCodeStr)
        local sql = string.format(" select proj_code,proj_module_name,count(proj_module_name) as proj_module_name_count,max(proj_link_review_time) as proj_link_review_time from tb_proj_link where proj_code in(%s) and ( proj_link_status=3 or proj_link_status=5 or proj_link_status = 7 or proj_link_status = 8) and proj_link_sync_time =0  and (proj_link_type =18 or proj_link_type = 19 ) group by proj_code,proj_module_name   ", projCodeStr)
...
function _PROJECT.project_update(proj_code, proj_tower_type, proj_tower_height, proj_lon, proj_lat, proj_addr, proj_establish_time, proj_bu_code, proj_bu_name, proj_company_code)

    local newValueTab = {}
    if proj_tower_type ~= nil then
        table.insert(newValueTab, string.format(" proj_tower_type = '%s' ", proj_tower_type))
    end
    if proj_tower_height ~= nil then
        table.insert(newValueTab, string.format(" proj_tower_height = '%s' ", proj_tower_height))
    end
    if proj_lon ~= nil then
        table.insert(newValueTab, string.format(" proj_lon = '%s' ", proj_lon))
    end
    if proj_lat ~= nil then
        table.insert(newValueTab, string.format(" proj_lat = '%s' ", proj_lat))
    end
    if proj_addr ~= nil then
        table.insert(newValueTab, string.format(" proj_addr = '%s' ", proj_addr))
    end
    if proj_establish_time ~= nil then
        table.insert(newValueTab, string.format(" proj_establish_time = '%s' ", proj_establish_time))
    end
    if proj_bu_code ~= nil then
        table.insert(newValueTab, string.format(" proj_bu_code = '%s' ", proj_bu_code))
    end
    if proj_bu_name ~= nil then
        table.insert(newValueTab, string.format(" proj_bu_name = '%s' ", proj_bu_name))
    end
    if proj_company_code ~= nil then
        table.insert(newValueTab, string.format(" proj_company_code = '%s' ", proj_company_code))
    end
    if #newValueTab < 1 then
        return false, nil
    end
    local newVauleStr = table.concat(newValueTab, " , ")

    local sql = string.format(" update tb_proj set %s where proj_code='%s' returning  proj_code ", newVauleStr, proj_code)
