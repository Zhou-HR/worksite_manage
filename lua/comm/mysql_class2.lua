...
function Mysql_CLass:close_conn()
    --关闭mysql连接封装
...
function Mysql_CLass:getAllNodes()
    --缓存没有命中，从数据库读取数据
...
function Mysql_CLass:query_item()
    ...
    if not err and str and not self.ismain then
        --如果正常生成str,则保存到缓存
...
function Mysql_CLass:getDateByDb()
    --缓存没有命中，从数据库读取数据
    ...
    for i, v in ipairs(self.menuTable) do
        ...
        for key in pairs(v) do
            if v[key] == ngx.null or v[key] == '' or v[key] == 'NULL' then
                --lua中返回的mysql数据null值为ngx.null
                v[key] = nil
            end
        end
        ...
        if v['parentId'] == 0 then
            ...
            for j, v2 in ipairs(self.menuTable) do
                if v['parentId'] == v2['Id'] then
                    --如果此节点找到父类，则放入零时数组中，并跳出循环
...
function Mysql_CLass:genSqlCmd(site)
    --创建数据库查询字符串
...
function Mysql_CLass:genJsonTable()
    --创建json字符串

    self.removeKey = -1

    while table.getn(self.menuTable) ~= 0 do
        --如果self.menuTable长度大于0，即还没有完全处理完
        local count = 0
        ...
        if self.hasFound == 0 then
            --如果没有找到父节点，则说明数组有错误，跳出循环
...
function Mysql_CLass:traversalTable(currentTable, parentId, item, num)
    ...
    for i, v in ipairs(currentTable) do
        ...
        elseif v.Children ~= nil then
            --如果没找到，并且此项有Children的，则递归去Children中查找
...
function Mysql_CLass:getRemoveIds(objtable, typestr)
    ...
    for i, v1 in ipairs(objtable) do
        --第一步，循环菜单
...
function Mysql_CLass:RemoveByIds(objtable, idsobj)
    -- 根据id数组删除对象中的内容
